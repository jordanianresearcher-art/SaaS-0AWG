  ...
  const onSubmit = async (values: FormValues) => {
    ...
    try {
      ...
      const { data, error } = await getSupabase().rpc('create_shop_with_owner', {
        ...
        p_default_payment_link: values.defaultPaymentLink || null,
    ...
    <div className="min-h-screen bg-zinc-50 px-4 py-10">
      <div className="mx-auto max-w-xl">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="mt-6 space-y-5 rounded-2xl border border-zinc-200 bg-white p-5" noValidate>
          ...
          <Field label="Deposit / payment link" htmlFor="ob-pay" error={errors.defaultPaymentLink?.message} hint="Optional. Customers use it to put down a deposit.">
            <Input id="ob-pay" type="url" {...register('defaultPaymentLink')} />
          </Field>
