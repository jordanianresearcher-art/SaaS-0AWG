...
§6 for what's still open in each. Migrations `0017`/`0018`/`0019` (as
written on this branch) are **not yet applied to production** — this
session has no Supabase CLI/token (standing limitation), so the shop
owner applies them the same way every prior migration in this repo has
been applied.

**Renumbered at merge time** (branch merged into the main development
branch, which had independently taken `0018`/`0019` for shop logo storage
and shop financing offers before this branch merged): `0017_inventory_merge.sql`
keeps its number — it was never applied anywhere, so there's no ordering
requirement forcing it later than the already-applied `0018`/`0019`.
`0018_import_legacy_inventory.sql` is renamed to `0020_import_legacy_inventory.sql`.
`0019_access_code_pgcrypto_search_path.sql` is dropped entirely: it repaired
an *already-applied, broken* `0017` on a database that hit the bug before
the fix landed, and production never applied a broken `0017` — the
corrected definitions below already carry the fix it existed to apply.
Every `0018`/`0019` reference below means, respectively, `0020` and
"folded into 0017" from here on.
...
was stale and has been corrected. `0017`/`0020` are safe to stack on top
of `0016` (`0018`/`0019` are already applied — see the renumbering note above).
...
### Slice 2 — Data migration (`0020_import_legacy_inventory.sql`) ✅ written, not yet run
...
1. **Apply `0017` then `0020` to production** (this session cannot — no
   Supabase CLI/token) and enable anonymous sign-ins in the Supabase
   dashboard's Auth settings. No separate repair migration is needed —
   production never applied a broken `0017`, so the corrected definitions
   land directly on the first (and only) apply.
