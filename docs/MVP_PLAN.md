...
merge, and `0019` by that migration's pgcrypto repair (see
`docs/INVENTORY_MERGE_PLAN.md`). Booking's migration is `0020`+
...
- `0017`, `0018`, `0019` → taken by the inventory/car-audio-inventory
  merge (`0017_inventory_merge.sql`, `0018_import_legacy_inventory.sql`,
  `0019_access_code_pgcrypto_search_path.sql` — see
  `docs/INVENTORY_MERGE_PLAN.md`). Booking's reservation moved to `0020`
  as a result — every "migration 0017"/"0019" reference below in the
  Booking spec means `0020` now.
- `0020` → Stream B (booking)
- `0021` → reserved, unassigned
...
Migration `0020`, types, `scheduling.ts` + its full test suite,
...
| --- | --- | --- | --- | --- | --- |
| 1 | Migration 0020, types, repository | *reading spec, scaffolding* | *reading spec, scaffolding* | *waiting on art* | — |
| 2 | `scheduling.ts` + tests, `book_appointment` RPC | Calendar shell on core | Booking page shell on core | Geometry from SVGs | — |
| 3 | Support both, apply 0020 to prod | Calendar day+week working | Service → vehicle → time flow | Web rendering + editor | — |
