...
booking. That number is now taken by `0017_inventory_merge.sql` (kept at
its original number rather than renumbered — see
`docs/INVENTORY_MERGE_PLAN.md`). `0018`/`0019` were already taken by shop
logo storage and shop financing offers, applied to production before the
inventory branch merged — its own `0018_import_legacy_inventory.sql`
was renumbered to `0020` to avoid the collision, and its
`0019_access_code_pgcrypto_search_path.sql` repair was dropped entirely:
0017's fresh-install definitions already carry the fix it existed to
apply, and production never ran the broken version. Booking's migration
is `0021`+ whenever that work starts. Whichever migration reaches this
repo next, apply it to production the same way (`supabase db push` or the
SQL editor) rather than letting it sit unapplied while screens ship
against it — and re-run the demo script against production again after
any stream that touches the schema merges.
...
Now the critical path. Full spec and the parity sequencing in §5. The
largest build. Owns all new booking files plus migration `0021`.
...
- `0017` → the inventory/car-audio-inventory merge (`0017_inventory_merge.sql`
  — see `docs/INVENTORY_MERGE_PLAN.md`)
- `0018`, `0019` → already taken and applied to production before the
  inventory merge landed: shop logo storage and shop financing offers
- `0020` → the inventory merge's data-import step
...
- `0030` → `0030_review_requests.sql` (the texted review ask: two columns on
  shops, the review_requests table, and three public-token RPCs). **Applied?
  No.**
- `0031`+ → reserved, unassigned. Stripe Connect deposits will take the next
...
Migration `0021`, types, `scheduling.ts` + its full test suite,
...
| --- | --- | --- | --- | --- | --- |
| 1 | Migration 0021, types, repository | *reading spec, scaffolding* | *reading spec, scaffolding* | *waiting on art* | — |
| 2 | `scheduling.ts` + tests, `book_appointment` RPC | Calendar shell on core | Booking page shell on core | Geometry from SVGs | — |
| 3 | Support both, apply 0021 to prod | Calendar day+week working | Service → vehicle → time flow | Web rendering + editor | — |
